...
class BalancedMultimodalMedicalImagePreprocessor:
    """
    Multimodal medical image preprocessing with class balancing
    """
    ...
    # [Keep all the previous methods: load_image, get_modality_specific_parameters, etc.]
    # ... (include all the methods from the previous multimodal code)
    ...
    def extract_brain_region(self, image, modality="MRI"):
        """Brain region extraction with modality-specific thresholding"""
        params = self.get_modality_specific_parameters(modality)
    ...
    def modality_specific_preprocessing(self, image_path, modality="MRI", save_quality_metrics=True):
        """Modality-specific preprocessing pipeline"""
...
def detect_modality_and_label(path):
...
def balance_dataset_strategy(all_images):
    """
    Apply strategic balancing to handle 4000 normal vs 10000 tumor
    """
    ...
    balanced_data = []
    preprocessor = BalancedMultimodalMedicalImagePreprocessor()
    ...
    for img_path, modality, label in tqdm(normal_images, desc="Augmenting normal images"):
        ...
        augmented_images = preprocessor.apply_data_augmentation(img_path, modality, label, 
                                                              num_augmentations=augmentation_factor-1)
        
        for aug_img in augmented_images:
            # Create synthetic file path for augmented image
            synthetic_path = f"synthetic_{img_path.stem}_{len(balanced_data)}.png"
...
def main():
    # Configuration
    BASE_DATASET = "/Users/jeissonparra/Library/CloudStorage/OneDrive-FloridaInternationalUniversity/Capstone/Multimodal_Deep Learning_for_Early_Detection_and_Classification_of_Brain_Tumors_Using_MRI_and_CT_Scans/Datasets/Original"
    OUTPUT_DATASET = "/Users/jeissonparra/Library/CloudStorage/OneDrive-FloridaInternationalUniversity/Capstone/Multimodal_Deep Learning_for_Early_Detection_and_Classification_of_Brain_Tumors_Using_MRI_and_CT_Scans/Datasets/Balanced_Multimodal"
    
    # Initialize preprocessor with balancing enabled
    preprocessor = BalancedMultimodalMedicalImagePreprocessor(target_size=(224, 224), apply_balancing=True)
    
    # Create output directories
    for modality in ["MRI", "CT"]:
        for split in ["train", "val", "test"]:
            for label in ["tumor", "normal"]:
                os.makedirs(f"{OUTPUT_DATASET}/{modality}/{split}/{label}", exist_ok=True)
    ...
    for modality in ["MRI", "CT"]:
        modality_path = Path(BASE_DATASET) / modality
    ...
    balanced_data = balance_dataset_strategy(all_images)
    ...
    # For synthetic images, we'll handle them differently
    ...
    # Process & Save
    print("\n🚀 Starting balanced multimodal preprocessing...")
    ...
    for modality, modality_splits in splits.items():
        ...
        for split_name, split_data in modality_splits.items():
            ...
            for img_path, label in tqdm(split_data, desc=f"Processing {modality} {split_name}"):
                try:
                    processed = preprocessor.modality_specific_preprocessing(img_path, modality)
                    
                    if processed is not None:
                        filename = f"{img_path.stem}_processed.npy"
                        output_path = f"{OUTPUT_DATASET}/{modality}/{split_name}/{label}/{filename}"
    ...
    for modality in ["MRI", "CT"]:
        for label in ["normal", "tumor"]:
            for split in ["train", "val", "test"]:
                split_path = Path(OUTPUT_DATASET) / modality / split / label
    ...
    for modality, counts in final_counts.items():
        ...
        print(f"  Ratio: {counts['tumor']/counts['normal']:.2f}:1" if counts['normal'] > 0 else "  Ratio: N/A")
    
    print(f"\n🎉 Balanced multimodal preprocessing complete!")
    print(f"📁 Balanced datasets saved in: {OUTPUT_DATASET}")
